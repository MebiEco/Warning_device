...
#define OTA_MIN_HEAP_LARGEST  (40 * 1024)
#define OTA_AZURE_STOP_TIMEOUT_MS  10000

EventGroupHandle_t otaEventGroup = NULL;
char g_ota_update_url[256]       = {0};
volatile bool bOtaPendingDownload = false;
...
static void log_ota_heap(const char *stage)
{
    ESP_LOGI(OTA_TAG, "[OTA HEAP @ %s] free=%" PRIu32 " internal=%" PRIu32 " largest=%" PRIu32,
             stage,
...
static bool user_ota_create_task(void)
{
    User_Ota_Init();
    ...
    log_ota_heap("before ota task create");
    if (xTaskCreatePinnedToCore(User_Ota_Task, "OTA Task", OTA_TASK_STACK, NULL, 5,
                                &s_ota_task_handle, 1) == pdPASS) {
        ESP_LOGI(OTA_TAG, "OTA task created on demand");
        return true;
    }

    ESP_LOGE(OTA_TAG, "Failed to create OTA task (stack=%d, largest=%" PRIu32 ")",
             OTA_TASK_STACK,
             (uint32_t)heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL));
    return false;
}
...
{
    (void)user_ota_create_task();
}

static void user_ota_rollback_prepare(void)
{
    bIsOtaActivated = false;
    IoTHubHandle.isNeedReinit = false;

    if (Azure_Process_Handle != NULL) {
        vTaskResume(Azure_Process_Handle);
        ESP_LOGW(OTA_TAG, "Azure process loop resumed after OTA abort");
    }
}

static bool user_ota_wait_azure_stopped(uint32_t timeout_ms)
{
    uint32_t elapsed = 0;
    ...
    return !IoTHubHandle.isAzureInitialized;
}

void User_Ota_Prepare_System(void)
{
    ESP_LOGW(OTA_TAG, "=== OTA priority mode: preparing system ===");

    Stop_Alarm();

    if (Azure_Process_Handle != NULL) {
        vTaskSuspend(Azure_Process_Handle);
        ESP_LOGI(OTA_TAG, "Azure process loop suspended");
    }
    ...
    log_ota_heap("after bt deinit");

    if (!user_ota_wait_azure_stopped(OTA_AZURE_STOP_TIMEOUT_MS)) {
        ESP_LOGW(OTA_TAG, "Azure still active after %d ms, continuing anyway",
                 OTA_AZURE_STOP_TIMEOUT_MS);
    }
    log_ota_heap("after azure stop");

    esp_coex_preference_set(ESP_COEX_PREFER_WIFI);
    vTaskDelay(pdMS_TO_TICKS(2000));
    log_ota_heap("before download");

    uint32_t largest = heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL);
    if (largest < OTA_MIN_HEAP_LARGEST) {
        ESP_LOGW(OTA_TAG, "Low largest heap block (%" PRIu32 " KB), using Azure task stack",
                 largest / 1024);
    }
}
...
bool User_Ota_Request_Update(const char *url)
{
    ...
    bOtaPendingDownload = true;
    ESP_LOGI(OTA_TAG, "OTA queued — Azure task will prepare and download");
    return true;
}
...
void User_Ota_Task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        {
            ESP_LOGW(OTA_TAG, "OTA command received! Target URL: %s", g_ota_update_url);
