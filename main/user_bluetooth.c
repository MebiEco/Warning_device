...
static int s_bt_retry_delay_ticks = 0;
  ...
  case ESP_BT_GAP_ACL_CONN_CMPL_STAT_EVT:
  {
    if (param->acl_conn_cmpl_stat.stat != ESP_BT_STATUS_SUCCESS) {
      ...
      if (bt_is_peer_addr(param->acl_conn_cmpl_stat.bda) &&
          s_a2d_state == APP_AV_STATE_CONNECTING) {
        bt_schedule_reconnect("ACL connect failed");
      }
    }
    break;
  }
...
static void bt_app_av_media_proc(uint16_t event, void *param)
{
  ...
  case APP_AV_MEDIA_STATE_STARTED:
    if (event == BT_APP_HEART_BEAT_EVT && !is_playing) {
      if (++s_idle_heartbeat_cnt >= BT_KEEPALIVE_HEARTBEATS) {
        s_idle_heartbeat_cnt = 0;
        bt_nudge_stream_keepalive();
      }
    } else if (is_playing) {
      s_idle_heartbeat_cnt = 0;
    }
...
static void bt_app_av_state_connected_hdlr(uint16_t event, void *param)
{
  ...
  case BT_APP_HEART_BEAT_EVT:
  {
    if (s_audio_cfg_received || user_bluetooth_is_connected()) {
      ...
      /* Push A2DP media stream even if audio cfg event was missed */
      esp_a2d_media_ctrl(ESP_A2D_MEDIA_CTRL_CHECK_SRC_RDY);
    }
    break;
  }
...
static void bt_av_hdl_avrc_ct_evt(uint16_t event, void *p_param)
{
  ...
  case ESP_AVRC_CT_CONNECTION_STATE_EVT:
  {
    ...
    if (rc->conn_stat.connected) {
      ...
      if (bt_is_peer_addr(rc->conn_stat.remote_bda) &&
          s_a2d_state == APP_AV_STATE_CONNECTED) {
        bt_on_speaker_lost("AVRC disconnected");
      }
    }
    break;
  }
...
bool user_bluetooth_wait_for_connect(uint32_t timeout_ms)
{
  ...
  while (elapsed < timeout_ms) {
    if (user_bluetooth_is_connected()) {
      ESP_LOGI(BT_AV_TAG, "A2DP connected — continuing boot.");
      return true;
    }
    if (s_avrc_connected) {
      ESP_LOGI(BT_AV_TAG, "AVRC linked to speaker — continuing boot.");
      return true;
    }
    if (s_bt_retry_count >= BT_CONNECT_RETRY_MAX) {
  ...
  if (user_bluetooth_is_link_ready()) {
    ESP_LOGI(BT_AV_TAG, "Bluetooth link ready at timeout boundary.");
    return true;
  }

  ESP_LOGW(BT_AV_TAG, "Bluetooth wait timeout (%" PRIu32 " ms).", timeout_ms);
  return false;
}
