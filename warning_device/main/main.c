...
void app_main(void)
{
  ...
  {
      ESP_LOGI(TAG, "================================");
      ESP_LOGI(TAG, " ENTERING CONFIG MODE (AP ONLY)");
  ...
  else
  {
      ...
      /* Initialize OTA event group early to avoid race conditions */
      ...
      /* Xác nhận firmware hiện tại OK (hủy rollback nếu đây là lần boot sau OTA) */
      User_Ota_Mark_Valid();

      if (xTaskCreatePinnedToCore(sd_read_task, "SD Read Task", 4 * 4096, NULL, 5, NULL, 0) == pdPASS)
      {
      ...
      if (xTaskCreatePinnedToCore(Bell_Task, "Bell Task", 2 * 4096, NULL, 4, NULL, 1) == pdPASS)
      {
      ...
      {
          ESP_LOGI(TAG, "Create OTA task successfully");
      }
      else
      {
          ESP_LOGE(TAG, "Create OTA task fail");
      }
  }
}
