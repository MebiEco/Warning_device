...
void Azure_Handle_Direct_Method_Data(cJSON *payload, DirectMethodResponse_t *response) 
{
    ...
    if ((code_item != NULL) && (data_item != NULL))
    {
        ...
        if(_code == _CMD_CODE_LOA) //code=100
        {
          ...
          if((Value != NULL) && (PondId != NULL) && (DeviceId != NULL) && (Duration != NULL) && (Repeat != NULL))
          {
            ...
            if (!PondId || !cJSON_IsNumber(PondId))
            {
            ...
            int Pondid   = PondId->valueint;
            int Deviceid = DeviceId->valueint;

            AlarmCmd_t cmd;
            cmd.file_id      = Pondid*100 + Deviceid;
            ...
            if (alarm_queue != NULL)
            {
              if (xQueueSend(alarm_queue, &cmd, 0) == pdTRUE)
              {
                ...
                ESP_LOGI("AZURE", "Queued alarm: %d (Pond:%d Dev:%d) Repeat:%d Dur:%ds",
                        cmd.file_id, Pondid, Deviceid, cmd.repeat_count, cmd.duration_sec);
              }
              else
              {
                // response->status = COMMAND_STATUS_DEVICE_ERROR;
